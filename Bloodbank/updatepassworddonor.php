...
if(isset($_GET['D_email']) && isset($_GET['resettoken']))
{
  date_default_timezone_set('Asia/colombo');
  $date =date('Y-m-d');
  $sql="SELECT * FROM `donor` WHERE`D_email`='$_GET[D_email]'AND `resettoken`='$_GET[resettoken]' 
  ...
  if($result)
  {
                    ...
                    <form class='form-box px-2'>
                        <div class='row'>
                            <div class='col-md-6 mt-md-0 mt-3'>
                                ...
                                <input type='hidden' name = 'email' value='$_GET[D_email]'>
                            </div>
                        </div> <br>
    ...
    window.location.href='Resetdonor.php';
    </script>";
  }
}
}
...
{
    $pass = $_POST['newpass'];
    $upadte = "UPDATE `donor` SET `D_password`='$pass',`resettoken`=NULL,`resettokenexpaire`=NULL WHERE `D_email`='$_POST[email]'";
    if(mysqli_query($conn,$upadte))
    {
        echo "<script>alert('password updated successfully');
        window.location.href='Donornew.php';
        </script>";
    }
    else{
        echo "<script>alert('invalid or Expaire date');
